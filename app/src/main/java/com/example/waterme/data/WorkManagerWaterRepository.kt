...
class WorkManagerWaterRepository(context: Context) : WaterRepository {
    ...
    override fun scheduleReminder(duration: Long, unit: TimeUnit, plantName: String) {
        ...
        // Создание OneTimeWorkRequest
        val workRequest = OneTimeWorkRequest.Builder(WaterReminderWorker::class.java)
            .setInputData(data)
            .setInitialDelay(duration, unit)
            .build()

        // Помещение запроса на выполнение в очередь WorkManager
        workManager.enqueueUniqueWork(
            plantName + duration,
            // Политика замены, REPLACE означает замену существующей работы
            ExistingWorkPolicy.REPLACE,
            // Объект OneTimeWorkRequest, созданный выше
            workRequest
        )
    }
}
